...
public class DoorBlock extends SolidBlock {
	...
	public void tick() {
		...
		if (openness < openLimit && !open && !blocksMotion) {
			if (level
					.containsBlockingEntity(x - 0.5, y - 0.5, x + 0.5, y + 0.5)) {
				openness = openLimit;
				return;
			}
		}
	...
	public boolean blocks(Entity entity) {
		double openLimit = 7 / 8.0;
		if (openness >= openLimit && entity instanceof Player)
			return blocksMotion;
		if (openness >= openLimit && entity instanceof Bullet)
			return blocksMotion;
		if (openness >= openLimit && entity instanceof OgreEntity)
			return blocksMotion;
		return true;
	}
}
